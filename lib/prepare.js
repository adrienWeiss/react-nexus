...
import isReactCompositeComponent from './util/isReactCompositeComponent';
...
async function prepareElement(element, context) {
  ...
  if(!isReactCompositeComponent(type)) {
    // Component might still be a functional component.
    await (typeof type[$prepare] === 'function' ? type[$prepare](props, context) : Promise.resolve());
